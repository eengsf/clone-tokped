...
import SubTrending from './Trending/SubTrending';
import { products } from '@/data/data';

function Trending() {
  const [rotate, setRotate] = useState(false);
  const data1 = products.slice(10, 18);
  const data2 = products.slice(10, 16);
  const data3 = products.slice(10, 14);
  ...
  return (
    <div className="flex flex-col xs:py-5 py-2 xs:border-b border-none gap-3">
      <div className="flex gap-3 items-end ">
        <h2 className="text-xl font-bold">Lagi trending, nih</h2>
        ...
        >
          <svg
            className={`unf-icon me-1 ${rotate ? 'animate-spin' : ''}`}
            viewBox="0 0 24 24"
            width="24"
            height="24"
      ...
      <div className="hidden lg:grid grid-cols-4 sm:gap-3 gap-2">
        <SubTrending data={data1} />
      </div>
      <div className="hidden md:grid lg:hidden grid-cols-3 sm:gap-3 gap-2">
        <SubTrending data={data2} />
      </div>
      <div className="grid md:hidden grid-cols-2 sm:gap-3 gap-2">
        <SubTrending data={data3} />
      </div>
    </div>
  );
}
